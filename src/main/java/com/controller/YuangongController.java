    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(yuangong !=null){
            ...
            BeanUtils.copyProperties( yuangong , view );//把实体数据重构到view中
                ...
                //级联表
                BumenEntity bumen = bumenService.selectById(yuangong.getBumenId());
                if(bumen != null){
                    BeanUtils.copyProperties( bumen , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
...
//                            yuangongEntity.setBumenId(Integer.valueOf(data.get(0)));   //部门 要改的
...
//                            yuangongEntity.setZhiwei(data.get(0));                    //职位 要改的
